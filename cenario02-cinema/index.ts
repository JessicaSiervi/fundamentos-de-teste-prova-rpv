...
function comprarIngressos(compra: ICompraIngresso): IResultadoCompra {
    ...
    function buscarFilme(): IFilme | undefined {
        return filmes.find(f => f.id === compra.filmeId)
    }

    function validar(filme: IFilme): boolean {
        if (compra.espectadores.length > 6) return false
        if (compra.sessao === 'noite') {
            const temMenorDe16 = compra.espectadores.some(e => e.idade < 16)
            if (temMenorDe16) return false
        }

        const temAbaixoClassificacao = compra.espectadores.some(e => e.idade < filme.classificacao)
        if (temAbaixoClassificacao) return false

        return true
    }

    function calcular(espectador: IEspectador, filme: IFilme): number {
        let valor = 40
        if (compra.diaSemana === 2) valor *= 0.70
        if (espectador.ehEstudante || espectador.idade >= 60) valor *= 0.50
        if (filme.eh3D) valor += 10

        return valor
    }

    const filme = buscarFilme()
    if (!filme) return { valorTotal: 0, quantidadeIngressos: 0, ehValida: false }

    if (!validar(filme)) return { valorTotal: 0, quantidadeIngressos: 0, ehValida: false }

    const valorTotal = compra.espectadores.reduce((acc, e) => acc + calcular(e, filme), 0)
